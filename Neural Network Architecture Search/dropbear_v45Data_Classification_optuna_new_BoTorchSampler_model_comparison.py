...
optuna_n_trials_MAX = 1
NUM_CLASSESS = 256

IF_RUNNING_FOR_MORE_ACCURATE_MODEL = True

optuna_sampler_tag = 'BoTorchSampler'

if IF_RUNNING_FOR_MORE_ACCURATE_MODEL:
    out_file_tag = '/path/to/directory/containing/results/more_accurate_model/'+optuna_sampler_tag + '_v1'
    result_path = '/path/to/directory/containing/results/more_accurate_model/main_results_'+optuna_sampler_tag
    rmse_cost_path = '/path/to/directory/containing/results/more_accurate_model/rmse_cost_'+optuna_sampler_tag
    acc_pin_ref_pred_filepath = '/path/to/directory/containing/results/more_accurate_model/'

    n_CNN_layers = 5
    CNN_filters_size_1 = 16
    n_LSTM_layers = 0
    LSTM_units_1 = 0
    n_MLP_layers = 5
    MLP_units_1 = 76
    ...
    out_file_tag = '/path/to/directory/containing/results/less_accurate_model/'+optuna_sampler_tag + '_v1'
    result_path = '/path/to/directory/containing/results/less_accurate_model/main_results_'+optuna_sampler_tag
    rmse_cost_path = '/path/to/directory/containing/results/less_accurate_model/rmse_cost_'+optuna_sampler_tag
    acc_pin_ref_pred_filepath = '/path/to/directory/containing/results/less_accurate_model/'
...
def create_model(n_CNN_layers, CNN_filters_size, n_LSTM_layers, LSTM_units, n_MLP_layers, MLP_units, feature_len):
    ...
    # final
    model.add(tf.keras.layers.Dense(NUM_CLASSESS, activation='softmax')) #sigmoid softmax
...
def process_predictions(optuna_suggested_param_tag, y_for_train, predictions_train, y_for_test, predictions_test):
    ...
    rmse_train = root_mean_squared_error(y_for_train, predictions_train)
    rmse_train = round(float(rmse_train), 4)
    snr_test = signaltonoise(y_for_test, predictions_test)
    snr_test = round(float(snr_test), 4)
    rmse_test = root_mean_squared_error(y_for_test, predictions_test)
...
def process_predictions_datasetwise(y_sep, ypred_sep, filename_sep, tag):
    ...
    rmse = root_mean_squared_error(y_sep, ypred_sep)
    rmse = round(float(rmse), 4)
...
def process_datasetwise_prediction(model, X_y_Full_seperate_datasets, optuna_suggested_param_tag, buckets_maps_discrete_to_original_pin_val):
    ...
    for xypair in X_y_Full_seperate_datasets:
        ...
        if 'Standard_Index_Set_test' in filename_sep:
            write_to_file(X_sep.tolist(), acc_pin_ref_pred_filepath+filename_sep + '_acc_data')
            write_to_file(current_y_orig.tolist(), acc_pin_ref_pred_filepath + filename_sep + '_pin_ref')
            write_to_file(ypred_sep.tolist(), acc_pin_ref_pred_filepath + filename_sep + '_pin_pred')
...
def prepare_dataset_for_each_trial(feature_len, acc_files):
    ...
    print('len breakpoints = ', len(breakpoints))
...
if __name__ == '__main__':
    ...
    study.optimize(objective, n_trials=optuna_n_trials)  # n_trials=250
