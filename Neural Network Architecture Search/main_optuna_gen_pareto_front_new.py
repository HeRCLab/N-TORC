...
n_trials = 360
dir_path = '/directory/containing/results/'
...
FILEPATH = '/directory/to/save/pareto/optimal/models/'
...
def gen_model_details(filepath):
    ...
    for line in lines:
        if line.startswith('for all_data_trial_test_n_CNN_layers='):
            ...
            line_details_model_cost = float(line_details_model_cost)*1000000
...
def prepare_pareto_fron_network(network_details):
    ...
    FILENAME = 'network_' + str(feature_len) + '_' + str(n_CNN_layers) + '_' + str(CNN_filters_size_1) + '_' + str(
        n_LSTM_layers) + '_' \
               + str(LSTM_units_1) + '_' + str(n_MLP_layers) + '_' + str(MLP_units_1)
...
def parse_save_pareto_front_networks(best_params):
    for item in best_params:
        ...
        prepare_pareto_fron_network(item.user_attrs)
